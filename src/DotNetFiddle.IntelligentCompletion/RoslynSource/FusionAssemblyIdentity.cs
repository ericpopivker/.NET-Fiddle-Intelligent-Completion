...
namespace Microsoft.CodeAnalysis
{
    internal sealed class FusionAssemblyIdentity
    {
        ...
        {
            System.Diagnostics.Debug.Assert(propertyId == PropertyId.NULL_PUBLIC_KEY_TOKEN || propertyId == PropertyId.NULL_PUBLIC_KEY);
            uint size = 0;
        ...
        internal static unsafe Version GetVersion(IAssemblyName nameObject)
        {
            ...
            if (hr != 0)
            {
                System.Diagnostics.Debug.Assert(hr == FUSION_E_INVALID_NAME);
                return null;
            }
        ...
        private static unsafe void SetProperty(IAssemblyName nameObject, PropertyId propertyId, string data)
        {
            ...
            {
                System.Diagnostics.Debug.Assert(data.IndexOf('\0') == -1);

                fixed (char* p = data)
                {
                    System.Diagnostics.Debug.Assert(p[data.Length] == '\0');
        ...
        internal static IAssemblyName ToAssemblyNameObject(AssemblyName name)
        {
            ...
            if (assemblyName != null)
            {
                ...
                {
                    throw new ArgumentException("Invalid characters in assemblyName", "name");
                }
            ...
            if (info != null)
            {
                ...
                {
                    throw new ArgumentException("Invalid characters in assembly name", "name");
                }
        ...
        internal static IAssemblyName ToAssemblyNameObject(string displayName)
        {
            ...
            System.Diagnostics.Debug.Assert(displayName != null);
            IAssemblyName result;
        ...
        internal static IAssemblyName GetBestMatch(IEnumerable<IAssemblyName> candidates, string preferredCultureOpt)
        {
            ...
            foreach (var candidate in candidates)
            {
                if (bestCandidate != null)
                {
                    ...
                    System.Diagnostics.Debug.Assert(candidateVersion != null);

                    if (bestVersion == null)
                    {
                        bestVersion = GetVersion(bestCandidate);
                        System.Diagnostics.Debug.Assert(bestVersion != null);
                    }
                    ...
                    if (cmp == 0)
                    {
                        if (preferredCultureOpt != null)
                        {
                            ...
                            System.Diagnostics.Debug.Assert(candidateCulture != null);

                            if (bestCulture == null)
                            {
                                bestCulture = GetCulture(candidate);
                                System.Diagnostics.Debug.Assert(bestCulture != null);
                            }
